...
_shop = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;disableSerialization;
...
_units = [];{
	...
	;}
	forEach nearestObjects [getMarkerPos "jail_marker",["Man"],15]; // 15 = distance from jail_marker// this adds all the prisioners around the jail_marker within 15 meters
	{
	_temp = format["%1", _x];
	if(["civ", _temp, false] call BIS_fnc_inString) then {	_list lbAdd format["%1", (_x getVariable["realname",name _x])];
	_list lbSetData [(lbSize _list)-1,str(_x)];
	};
	}
	forEach _units;
	ctrlSetText[8104,"Connected to system..."];
	if(((lbSize _list)-1) == -1) then{	_list lbAdd "No prisoners...";
};